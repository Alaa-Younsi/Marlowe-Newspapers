...
export default function TVShows() {
  const allShows = getArticlesByCategory('tv-shows');
  const { query, setQuery, results } = useSearch(allShows);
  const seo = buildSeoMeta({
    title: 'Television | Marlowe Newspapers',
    description: 'TV show reviews and analysis by Alaa Younsi. The small screen, unshrunk.',
  });
  ...
  return (
    <>
      ...
      <div className="max-w-broadsheet mx-auto px-4 md:px-8 py-8">
        <SectionHeader left="TELEVISION" right={`${results.length} REPORTS`} />

        <p className="category-dateline">
          — Dispatches from the Small Screen by Philip Marlowe —
        </p>
        ...
        <div className="mb-6">
          <input
            ...
            aria-label="Search TV show articles"
          />
        </div>
        ...
        {results.length > 0 ? (
          ...
          >
            <ColumnsGrid articles={results} sectionTitle="TELEVISION" />
          </motion.div>
        ) : (
