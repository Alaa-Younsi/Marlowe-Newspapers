...
export default function NovelsMangas() {
  const allItems = getArticlesByCategory('novels-mangas');
  const { query, setQuery, results } = useSearch(allItems);
  const seo = buildSeoMeta({
    title: 'Novels & Manga | Marlowe Newspapers',
    description: 'Book and manga reviews by Alaa Younsi. Words that refuse to stay on the page.',
  });
  ...
  return (
    <>
      ...
      <div className="max-w-broadsheet mx-auto px-4 md:px-8 py-8">
        <SectionHeader left="NOVELS &amp; MANGA" right={`${results.length} REPORTS`} />

        <p className="category-dateline">
          — Literary Reviews &amp; Manga Analysis by Philip Marlowe —
        </p>
        ...
        <div className="mb-6">
          <input
            ...
            aria-label="Search novel and manga articles"
          />
        </div>
        ...
        {results.length > 0 ? (
          ...
          >
            <ColumnsGrid articles={results} sectionTitle="NOVELS &amp; MANGA" />
          </motion.div>
        ) : (
