...
export default function Movies() {
  const allMovies = getArticlesByCategory('movies');
  const { query, setQuery, results } = useSearch(allMovies);
  const seo = buildSeoMeta({
    title: 'Motion Pictures | Marlowe Newspapers',
    description: 'Film reviews and analysis by Alaa Younsi — from noir classics to contemporary cinema.',
  });
  ...
  return (
    <>
      ...
      <div className="max-w-broadsheet mx-auto px-4 md:px-8 py-8">
        <SectionHeader left="MOTION PICTURES" right={`${results.length} REPORTS`} />

        <p className="category-dateline">
          — Critical Reports on the Moving Image by Philip Marlowe —
        </p>
        ...
        <div className="mb-6">
          <input
            ...
            aria-label="Search film articles"
          />
        </div>
        ...
        {results.length > 0 ? (
          ...
          >
            <ColumnsGrid articles={results} sectionTitle="MOTION PICTURES" />
          </motion.div>
        ) : (
